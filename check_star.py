...
def test_from_file(path, show_windows: bool):
    ...
    if show_windows:
        ...
        if roi_img.size == 0:
            h, w = frame.shape[:2]
            print(f"WARNING: ROI {roi} is outside image bounds ({w}x{h}). "
                  "Adjust TEST_STAR_* constants in check_star.py.")
        else:
...
def watch_live(seconds: float, show_windows: bool, device: int | None = None):
    dev = device if device is not None else CAPTURE_INDEX
    cap = cv2.VideoCapture(dev, cv2.CAP_DSHOW)
    cap.set(cv2.CAP_PROP_FRAME_WIDTH, 1280)
    cap.set(cv2.CAP_PROP_FRAME_HEIGHT, 720)

    if not cap.isOpened():
        print(f"Could not open capture device {dev}")
        print("Hint: If OBS is using the capture card, start OBS Virtual Camera")
        print("      and try: --device 1  (or 2, 3, ...)")
        return 2
...
def main():
    ...
    parser.add_argument("--device", type=int, default=None,
                        help="Capture device index (default: 0, try 1 for OBS Virtual Camera)")
    ...
    if args.watch_seconds:
        sys.exit(watch_live(args.watch_seconds, args.show, args.device))

    sys.exit(watch_live(999999, True, args.device))
