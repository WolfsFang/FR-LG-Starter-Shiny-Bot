// main.c — Pico 2 W: Wi-Fi STA + HTTP :8080 + USB HID
// GET  /ping   -> pong
// GET  /ready  -> ready
...
//
// Build:
// cmake .. -DPICO_BOARD=pico2_w -DWIFI_SSID='"yourssid"' -DWIFI_PASSWORD='"yourpass"'
// cmake --build . -j
...
int main(void) {
    ...
    if (cyw43_arch_init()) {
        // Init failed — no LED available, just hang
        while (true) {
            sleep_ms(1000);
        }
    }
    ...
    // Try connecting to WiFi with retries
    int rc = -1;
    for (int attempt = 0; attempt < 5; attempt++) {
        // Blink attempt number (1-5 blinks = which attempt)
    ...
    if (rc != 0) {
        // All attempts failed — slow blink forever
        while (true) {
            cyw43_arch_gpio_put(CYW43_WL_GPIO_LED_PIN, 1);
            sleep_ms(1000);
            cyw43_arch_gpio_put(CYW43_WL_GPIO_LED_PIN, 0);
    ...
    while (true) {
        ...
        __wfi();  // sleep until any interrupt (USB SOF ~1ms, cyw43 timer ~2ms)
    }
}