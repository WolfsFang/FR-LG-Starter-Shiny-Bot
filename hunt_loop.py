...
SHELL_SCRIPT = BASE_DIR / "run_sequence.ps1"
...
CAPTURE_DEVICE = None  # None = use check_star.py default (device 0), set to 1 for OBS Virtual Camera
...
def run_shell_script():
    print("\nRunning run_sequence.ps1 ...")
    result = subprocess.run(
        ["powershell.exe", "-ExecutionPolicy", "Bypass", "-File", str(SHELL_SCRIPT)],
        cwd=BASE_DIR
    )
...
def main():
    ...
    try:
        while not stop_requested:
            ...
            if shell_rc == 2:
                print("Pico unreachable - retrying in 5 seconds...")
                time.sleep(5)
                continue
            elif shell_rc != 0:
                print(f"run_sequence.ps1 failed with code {shell_rc}")
                break
